// --- CRUD FACTURE-clients ---
const SERVICE_COMMERCIAL = 'commercial'; // le service commercial est le service qui gère les factures clients
...
const PORT = 3013; // Les factures-clients écoute sur le port 3013
...
const generateInvoiceNumber = async (type) => {
    const table = type === 'fournisseur' ? 'factures_clients' : 'factures_clients';
...
app.post('/api/factures-clients', async (req, res) => {
    try {
        const { client_id, date, montant_ht, tva, frais_douane, statut } = req.body;
        const numero = await generateInvoiceNumber('client');
        ...
        const [result] = await pool.execute(
            `INSERT INTO factures_clients (client_id, numero, date, montant_ht, tva, frais_douane, montant_ttc, statut) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
            [client_id, numero, date, montant_ht, tva, frais_douane || 0, montant_ttc, statut || 'en attente']
        );
...
// Modification des montants de facture-clients ou de statut
// et depend sur la valeur de statut, on effectue des transcations surla table paiment-fournisseur

app.put('/api/factures-clients/:id', async (req, res) => {
    ...
    try {
        ...
        if (statut === 'payée') {
            ...
            const [payments] = await connection.execute('SELECT SUM(montant) as total FROM paiements_clients WHERE facture_id = ?', [id]);
            const deja_paye = payments[0].total || 0;
            const reste_a_payer = montant_ttc - deja_paye;

            if (reste_a_payer > 0) {
                await connection.execute(
                    `INSERT INTO paiements_clients (facture_id, date, montant, mode_paiement) VALUES (?, NOW(), ?, ?)`,
            ...
            await connection.execute('DELETE FROM paiements_clients WHERE facture_id = ?', [id]);
        }
...
app.get('/api/factures-clients/:id/paiements', async (req, res) => {
    try {
        const { id } = req.params;

        // On récupère d'abord les infos de la facture pour le contexte
        const [facture] = await pool.execute(
            'SELECT numero, montant_ttc, statut FROM factures_clients WHERE id = ?', 
            [id]
        );
...
app.delete('/api/factures-clients/:id', async (req, res) => {
    try {
        await pool.execute('DELETE FROM factures_clients WHERE id = ?', [req.params.id]);
...
app.delete('/api/paiements-clients/:paiement_id', async (req, res) => {
    const connection = await pool.getConnection();
    try {
        await connection.beginTransaction();
        ...
        const [paiement] = await connection.execute(
            'SELECT facture_id FROM paiements_clients WHERE id = ?', 
            [paiement_id]
        );
        ...
        await connection.execute('DELETE FROM paiements_clients WHERE id = ?', [paiement_id]);

        // 3. Recalculer le nouveau statut de la facture
        const [totalPaiements] = await connection.execute(
            'SELECT SUM(montant) as total FROM paiements_clients WHERE facture_id = ?', 
            [facture_id]
        );
        const [factureInfo] = await connection.execute(
            'SELECT montant_ttc FROM factures_clients WHERE id = ?', 
            [facture_id]
        );
        ...
        await connection.execute(
            'UPDATE factures_clients SET statut = ? WHERE id = ?',
...
app.get('/api/factures-clients/:id/pdf', async (req, res) => {
  try {
    const { id } = req.params;

    const [rows] = await pool.execute(`
      SELECT f.*, fr.nom as client_nom, fr.adresse as client_adresse 
      FROM factures_clients f
      JOIN clients fr ON f.client_id = fr.id
      WHERE f.id = ?`,
      [id]
    ...
    const doc = new PDFDocument({
      ...
      size: 'A4',
      info: {
        Title: `Facture Client ${facture.numero}`,
        Author: 'SERVICE IMMOBILIER S.C.',
      }
    });
    ...
    doc.fillColor(COLORS.darkText)
       .font('Helvetica-Bold')
       .fontSize(18)
       .text('FACTURE Client', 0, headerY + 8, { width: RIGHT, align: 'right' });
    ...
    doc.fillColor(COLORS.darkText).font('Helvetica-Bold').fontSize(9)
       .text('SERVICE IMMOBILIER S.C.', MARGIN, infoY + 13);
    ...
    doc.fillColor(COLORS.darkText).font('Helvetica-Bold').fontSize(9)
       .text(facture.client_nom || 'N/A', col2X, infoY + 13);
    doc.fillColor(COLORS.bodyText).font('Helvetica').fontSize(8)
       .text(facture.client_adresse || 'Adresse non renseignée', col2X, infoY + 25, { width: 165 });
    ...
    doc.fillColor(COLORS.lightMuted).font('Helvetica').fontSize(7.5)
       .text(
         'SERVICE IMMOBILIER S.C.  —  RC : 123456  |  ICE : 001234567000089  |  IF : 12345678',
...
app.listen(PORT, () => {
    console.log(`api Facture-clients running on http://localhost:${PORT}`);
});